...
import { processPromptFile } from '../lib/legacy/promptProcessor.legacy'; // Adjust if your compiled JS is elsewhere or use ts-node/tsx
import { logger } from '../lib/utils/logger'; // Import logger for mocking
...
describe('processPromptFile', () => {
    const initialIndent = 0; // Define initial indent for test calls
    ...
    // Mock logger methods before each test
    beforeEach(() => {
        // Mock the actual logger methods, matching the new signature
        vi.spyOn(logger, 'warn').mockImplementation((indent, ...args) => {});
        vi.spyOn(logger, 'error').mockImplementation((indent, ...args) => {});
        // Keep debug/info mocks if needed for other tests, or remove if unused
        vi.spyOn(logger, 'debug').mockImplementation((indent, ...args) => {});
        vi.spyOn(logger, 'info').mockImplementation((indent, ...args) => {});
    });

    // Restore original logger methods after each test
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    it('should process a file with no includes', async () => {
        ...
        const result = await processPromptFile(filePath, BASE_DIR, initialIndent); // Add indent
    ...
    it('should return error comment for invalid line range (start > end)', async () => {
        ...
        expect(logger.error).toHaveBeenCalled(); // Check logger mock
    });
    ...
    it('should handle regex range with only startRegex (includes till end)', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(expect.any(Number), expect.stringContaining('Start regex /^## START$/ not found'));
    });
    ...
    it('should handle regex range where endRegex is not found (includes till end)', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(expect.any(Number), expect.stringContaining('End regex /^## NOT_FOUND$/ not found after start regex'));
    });
    ...
    it('should return error comment for non-matching startRegex', async () => {
        ...
        const result = await processPromptFile(filePath, BASE_DIR, initialIndent); // Add indent
        expect(result).toContain('<!-- Error including common/regex_target.md#startRegex=/^## NOT_FOUND$/endRegex=/^## END$/: Start regex /^## NOT_FOUND$/ not found');
        expect(logger.error).toHaveBeenCalled(); // Check logger mock
    });
    ...
    it('should handle empty content selection between regexes', async () => {
        ...
        const result = await processPromptFile(filePath, BASE_DIR, initialIndent); // Add indent
        expect(result).toBe('');
        expect(logger.warn).toHaveBeenCalledWith(expect.any(Number), expect.stringContaining('Start and end regex resulted in empty or invalid content selection'));
    });
    ...
    it('should return error comment for file not found', async () => {
        ...
        const result = await processPromptFile(filePath, BASE_DIR, initialIndent); // Add indent
        expect(result).toContain('<!-- Error including non_existent_file.md: File not found: ');
        expect(result).toContain('non_existent_file.md -->');
        expect(logger.error).toHaveBeenCalled(); // Check logger mock
    });
    ...
    it('should return warning comment and skip self-inclusion', async () => {
        ...
        const result = await processPromptFile(filePath, BASE_DIR, initialIndent); // Add indent
        expect(result).toBe('Self: <!-- Warning: Skipped self-inclusion of ./self_include.md in self_include.md -->');
        expect(logger.warn).toHaveBeenCalledWith(expect.any(Number), expect.stringContaining('Skipped self-inclusion'), expect.stringContaining('self_include.md'));
    });
